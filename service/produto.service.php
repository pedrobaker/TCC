...
class ProdService{
	...
	function create(){
		try{
			$sql = 'INSERT INTO produtos(id, descr, prCom, prVen, tipo, estoque, idEmp) VALUES (:id, :descr, :prCom, :prVen, :tipo, :estoque, :idEmp)';
	...
	function show($atributo, $valorAtr){
		try{
			...
			$stmt->execute();*/
			if ($acao[1]=='mostrar') {
				$sql = 'SELECT id, descr, prCom, prVen, tipo, estoque, idEmp FROM produtos WHERE id = :id ORDER BY id';
				$stmt = $this->conexao->prepare($sql);
				$stmt->bindValue(':id', $this->produto->__get('id'));
				$stmt->execute();
			}
			...
			if($acao[1]=='listar'){
				$sql = 'SELECT id, descr, prCom, prVen, tipo, estoque, idEmp FROM produtos WHERE :atributo = :valorAtr ORDER BY descr';
				$stmt = $this->conexao->prepare($sql);
	...
	function update(){
		try {
			$sql =	'UPDATE produtos SET descr = :descr, prCom = :prCom, prVen = :prVen, tipo = :tipo, estoque = :estoque, idEmp = :idEmp WHERE id = :id';
			$stmt = $this->conexao->prepare($sql);
